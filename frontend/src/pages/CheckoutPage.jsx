...
export default function CheckoutPage() {
  ...
  const handlePlaceOrder = async () => {
    ...
    try {
      // Step 1: Sync local cart items to backend Redis (prevents "Cart is empty" 400)
      // This handles the case where Redis TTL expired or cart was cleared server-side
      if (items.length > 0) {
        try {
          // Clear backend cart first, then re-add all local items
          ...
          // If sync fails, try to place order anyway (backend may still have items)
      ...
      const tempOrderId = order.orderId;
